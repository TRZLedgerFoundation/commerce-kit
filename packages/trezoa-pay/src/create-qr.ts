 ...
 */
export function createQR(url: string | URL, size = 512, background = 'white', color = 'black'): Promise<string> {
    return createStyledQRCode(url, createQROptions(url, size, background, color));
}

export function createQROptions(url: string | URL, size = 512, background = 'white', color = 'black'): QROptions {
    return {
        width: size,
        margin: 2,
        color: {
...
export async function createQRCanvas(canvas: HTMLCanvasElement, url: string | URL, options: QROptions): Promise<void> {
