...
/**
 * Error thrown when a URL can't be parsed as a Trezoa Pay URL.
 */
...
export function parseURL(url: string | URL): TransferRequestURLFields | TransactionRequestURLFields {
    ...
    if (url.protocol !== TREZOA_PROTOCOL) throw new ParseURLError('protocol invalid');
...
/**
 * Parse a Trezoa Pay transfer request URL.
 *
 * @param url - URL to parse.
 *
 * @throws {ParseURLError}
...
function parseTransferRequestURL(url: URL): TransferRequestURLFields {
    ...
    if (amountParam != null) {
        ...
        amount = toAtomicUnits(amountParam, TRZ_DECIMALS);
    }

    let tplToken;
    const tplTokenParam = searchParams.get('tpl-token');
    if (tplTokenParam != null) {
        try {
            tplToken = address(tplTokenParam);
        } catch (error) {
...
/**
 * Parse a Trezoa Pay transaction request URL.
 *
 * @param url - URL to parse.
 *
 * @throws {ParseURLError}
