        ...
        if (verified && expectedRecipient && expectedAmount != null) {
            ...
            if (!verified && expectedMint) {
                try {
                    ...
                    const isMatch = (acc: any) =>
                        acc &&
                        txData.meta?.postTokenBalances?.some(tb => {
                            ...
                            return (
                                key === acc.toString() && tb.mint === mintAddr.toString() && amount >= expectedAmount
                            );
                        });
...
export function createCommercePaymentRequest(request: OrderRequest) {
    ...
    const totalAmount = items ? items.reduce((sum, item) => sum + item.price, 0) : amount;
