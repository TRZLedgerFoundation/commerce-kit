import { createTrezoaPayRequest, TrezoaPayRequestOptions, toMinorUnits } from '@trezoa-commerce/headless';
import { Recipient } from '@trezoa-commerce/trezoa-pay';
import { useMemo } from 'react';
import { Currency, CurrencyMap } from '../types';
import { useAsync } from './use-async';

export interface TrezoaPayQROptions extends TrezoaPayRequestOptions {
    label?: string;
    message?: string;
}
...
export function useTrezoaPay(recipient: string, amount: number, currency: Currency, opts?: TrezoaPayQROptions) {
    ...
    const asyncRequestCreation = useMemo(
        () =>
            requestParams
                ? async () => {
                      const request = await createTrezoaPayRequest(requestParams.paymentData, requestParams.qrOptions);
