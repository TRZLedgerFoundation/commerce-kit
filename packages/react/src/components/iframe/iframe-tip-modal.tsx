...
export const IframeTipModalContent = memo<TipModalContentProps>(({ config, theme, onPayment, onCancel }) => {
                ...
                <div className={`sc-step ${state.currentStep === 'form' ? 'active' : ''}`}>
                    <div className="sc-content">
                        ...
                        >
                            {state.isProcessing
                                ? 'Processing...'
                                : `Pay ${formatPayLabel(state.selectedCurrency, computed.finalAmount || 0)}`}
                        </ActionButton>
                    </div>
                </div>
