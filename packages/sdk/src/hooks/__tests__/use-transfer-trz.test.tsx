...
import { useTransferTRZ } from '../use-transfer-trz';
...
vi.mock('../../core/transaction-builder', () => ({
    createTransactionBuilder: vi.fn(() => ({
        transferTRZ: vi.fn().mockImplementation(async (to: string, amount: bigint) => {
...
describe('useTransferTRZ', () => {
    ...
    it('should initialize with provided input values', () => {
        const { result } = renderHook(() => useTransferTRZ('test-address', '1.5'), {
            wrapper: createWrapper(),
        });
    ...
    describe('transferTRZ function', () => {
        it('should successfully transfer SOL', async () => {
            ...
            const transferOptions = {
                ...
                amount: MOCK_LAMPORTS.ONE_TRZ,
            };

            let transferResult: unknown;

            await act(async () => {
                transferResult = await result.current.transferTRZ(transferOptions);
            });
        ...
        it('should handle transfer errors', async () => {
            ...
            await act(async () => {
                try {
                    await result.current.transferTRZ(transferOptions);
                } catch (error) {
    ...
    describe('transferFromInputs function', () => {
        it('should transfer using input values', async () => {
            ...
            expect(transferResult.amount).toBe(MOCK_LAMPORTS.ONE_TRZ);
        });
    ...
    describe('handleSubmit function', () => {
        it('should prevent default and transfer when inputs are valid', async () => {
            ...
            expect(submitResult.amount).toBe(MOCK_LAMPORTS.HALF_TRZ);
        });
        ...
        it('should work without preventDefault method', async () => {
            ...
            expect(submitResult).toBeDefined();
            expect(submitResult.amount).toBe(MOCK_LAMPORTS.POINT_ONE_TRZ);
        });
    });
    ...
    describe('reset function', () => {
        it('should reset the mutation state', async () => {
            ...
            await act(async () => {
                await result.current.transferTRZ({
                    to: MOCK_ADDRESSES.WALLET_2,
                    amount: MOCK_LAMPORTS.ONE_TRZ,
                });
            });
    ...
    describe('loading states', () => {
        it('should show loading state during transfer', async () => {
            ...
            expect(result.current.transferTRZ).toBeDefined();

            // Start transfer - don't await to catch loading state
            let transferPromise: Promise<unknown>;

            act(() => {
                transferPromise = result.current.transferTRZ({
                    to: MOCK_ADDRESSES.WALLET_2,
                    amount: MOCK_LAMPORTS.ONE_TRZ,
                });
            });
