...
import { address, type Address, type TransactionSigner } from '@trezoa/kit';
import { createInvalidator } from '../utils/invalidate';

/**
 * Converts TRZ amount string to lamports with precise string-based arithmetic
 * Avoids floating-point precision issues by handling decimal conversion manually
 */
function convertSOLToLamports(trzAmount: string): bigint {
    // Validation: empty or whitespace
    if (!trzAmount || !trzAmount.trim()) {
        throw new Error('Amount cannot be empty');
    }

    const trimmed = trzAmount.trim();
    ...
    // Convert to BigInt - integer part shifted left by 9 decimal places (1e9 lamports = 1 TRZ)
...
export interface UseTransferTRZReturn {
    transferTRZ: (options: TransferSOLOptions) => Promise<TransferSOLResult>;
    isLoading: boolean;
    error: Error | null;
    ...
    /** Form submission handler that transfers TRZ using current input values */
    handleSubmit: (event?: { preventDefault?: () => void }) => Promise<TransferSOLResult | undefined>;
    /** Shortcut to transfer TRZ using current input values */
    transferFromInputs: () => Promise<TransferSOLResult | undefined>;
}

export function useTransferTRZ(initialToInput = '', initialAmountInput = ''): UseTransferTRZReturn {
    ...
    const mutation = useMutation<TransferSOLResult, Error, TransferSOLOptions>({
        mutationFn: async (options: TransferSOLOptions): Promise<TransferSOLResult> => {
            ...
            // Use shared TRZ transfer implementation
            return await transactionBuilder.transferTRZ(to, amount, signer);
        },
    ...
    return {
        transferTRZ: (options: TransferSOLOptions) => mutation.mutateAsync(options),
