...
export class ArcError extends Error {
    ...
    constructor(message: string, code: ArcErrorCode, context: Partial<ArcErrorContext> = {}, originalError?: Error) {
        super(message);
        this.name = 'ArcError';
        this.code = code;
...
export class ArcRetryManager {
    ...
    async executeWithRetry<T>(
        ...
        while (attempt < (retryConfig.maxAttempts ?? 0)) {
            try {
                ...
                console.warn(
                    `[Arc] Operation failed (attempt ${attempt}/${retryConfig.maxAttempts}), retrying in ${delay}ms:`,
                    lastError.message,
                );
    ...
    private calculateDelay(attempt: number, config: ArcRetryConfig, error: ArcError): number | false {
